-- 026_tenant_chamcong_bangluong.sql — CHAM_CONG + BANG_LUONG trong database ĐƠN VỊ-NĂM
--                                      (SCHEMA_VERSION = 19)
--
-- CHẠY CHO: mọi database ĐƠN VỊ-NĂM <MÃ>_<NĂM>. VaCauTrucService tự chạy khi vào màn.
--     sqlcmd -S <server> -d <MÃ>_<NĂM> -i 026_tenant_chamcong_bangluong.sql
--
-- Spec: docs/THUE/HOPDONG/SPEC-HOP-DONG-CHAM-CONG-LUONG.md (mục 6.3, 6.4).
-- Đi cùng script 025 (NHAN_SU + HOP_DONG) — hai bảng dưới đây đều trỏ về NHAN_SU.
--
...
IF DB_NAME() NOT IN ('KT2000_Base', 'KT2000_Master', 'master', 'msdb', 'model', 'tempdb')
   ...
   AND NOT EXISTS (SELECT 1 FROM SCHEMA_VERSION WHERE Ver = 19)
    INSERT INTO SCHEMA_VERSION (Ver) VALUES (19);
GO
